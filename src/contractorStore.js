...
export async function contractorSignUp({email,password,name,company,phone,city,state}){
  ...
  const profileRes=await fetch(`${SUPABASE_URL}/rest/v1/lucas_contractors`,{
    method:'POST',
    ...
    body:JSON.stringify({user_id:session.user.id,email,name:name||'',company:company||'',phone:phone||'',city:city||'',state:state||''})
  });
  if(!profileRes.ok){const err=await profileRes.text();contractorLogout();throw new Error(err||'Não foi possível concluir o cadastro.');}
...
export async function fetchContractorProfile(session=getContractorSession()){
  ...
  const res=await fetch(`${SUPABASE_URL}/rest/v1/lucas_contractors?user_id=eq.${encodeURIComponent(session.user.id)}&select=*`,{
...
export async function updateContractorProfile(profile,session=getContractorSession()){
  ...
  const res=await fetch(`${SUPABASE_URL}/rest/v1/lucas_contractors?user_id=eq.${encodeURIComponent(session.user.id)}`,{
    method:'PATCH',headers:{apikey:SUPABASE_KEY,Authorization:`Bearer ${session.access_token}`,'Content-Type':'application/json',Prefer:'return=representation'},body:JSON.stringify(allowed)
  });
...
export async function fetchContractorJobs(session=getContractorSession()){
  ...
  const res=await fetch(`${SUPABASE_URL}/rest/v1/lucas_contractor_jobs?contractor_id=eq.${encodeURIComponent(session.user.id)}&select=*&order=event_date.asc.nullslast,created_at.desc`,{
    headers:{apikey:SUPABASE_KEY,Authorization:`Bearer ${session.access_token}`}
  });
